...
function slt_fs_scripts() {
	wp_enqueue_script( 'slt-file-select', plugins_url( '/slt-file-select/slt-file-select.js' ), array( 'jquery', 'media-upload', 'thickbox' ) );
...
// Output form button
function slt_fs_button( $name, $value, $label = 'Select file', $preview_size = 'thumbnail', $removable = true ) { ?>
	<div>
		<input type="button" class="button-secondary slt-fs-button" value="<?php echo esc_attr( $label ); ?>" />
		<?php if ( $value && $removable ) { ?>
			&nbsp;&nbsp;<input type="checkbox" name="<?php echo esc_attr( $name ); ?>_remove" value="1" class="slt-fs-remove" /> <label for="<?php echo esc_attr( $name ); ?>_remove"><?php _e( 'Remove' ); ?></label>
		<?php } ?>
